...
def cargar_y_preparar_todos_los_datos(nombre_archivo_csv="FIFA.csv"):
    try:
        ...
        print(f"Datos cargados exitosamente desde '{nombre_archivo_csv}'.")
        ...
        print("Fechas y horas organizadas.")

        tabla_maestra_de_tweets["Primer_Nombre"] = tabla_maestra_de_tweets["Name"].apply(
            lambda nombre_usuario: nombre_usuario.split()[0] if pd.notna(nombre_usuario) else None
        )

        tabla_maestra_de_tweets["Sexo"] = tabla_maestra_de_tweets["Primer_Nombre"].apply(
            lambda nombre_a_detectar: adivinar_sexo_del_usuario(nombre_a_detectar, detector_de_genero)
        )
        print(" Sexo estimado para cada usuario.")

        tabla_maestra_de_tweets['Plataforma'] = simplificar_nombre_de_plataforma(
            tabla_maestra_de_tweets['Source']
        )
        print(" Plataformas de tuiteo también organizadas.")
...
def analizar_plataforma_mas_usada(datos_para_analisis):
    ...
    print(f" ¡Contamos los tweets por plataforma! Encontramos {len(conteo_total_de_tweets_por_plataforma)} tipos de plataformas principales.")
    ...
    for indice_barra, cantidad_tweets in enumerate(conteo_total_de_tweets_por_plataforma):
        objeto_eje_del_grafico.text(indice_barra, cantidad_tweets + 1, str(cantidad_tweets), ha='center', va='bottom')
...
def analizar_sexo_y_plataforma(datos_para_analisis):
    ...
    print(" Conteo de tweets por sexo y plataforma listo.")
...
def calcular_promedio_palabras_por_tweet(datos_para_analisis):
    ...
    datos_para_analisis['Conteo_de_Palabras'] = datos_para_analisis['Tweet'].dropna().apply(
        lambda texto_tweet: len(str(texto_tweet).split())
    )

    promedio_final_de_palabras = datos_para_analisis['Conteo_de_Palabras'].mean()
    print(f" ¡Promedio de palabras por tweet calculado!")
...
def encontrar_tweets_extremos(datos_para_analisis):
    ...
    print(" Tweets más largo y más corto identificados")
...
def analizar_likes_promedio(datos_para_analisis):
    ...
    plt.xticks(rotation=45, ha='right')
    for indice_plataforma, valor_promedio_plataforma in enumerate(likes_promedio_por_plataforma):
        eje_grafico_plataforma.text(indice_plataforma, valor_promedio_plataforma + 0.5, f'{valor_promedio_plataforma:.1f}', ha='center', va='bottom')
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    if datos_completos_de_tweets.empty:
        print("No hay datos para analizar. No se encontró el archivo CSV 'FIFA.csv' en la carpeta!")
    else:
